...
$no = 1;
$query = mysqli_query($conn, "SELECT * FROM `dat_leasing` WHERE `jenis_leasing`='ARMADA' AND `Keterangan` NOT LIKE 'TERJUAL' AND `tgl_pajak` <= '$dat' ORDER BY `tgl_pajak` ASC") or die(mysqli_error($conn));
...
?>
	<tr>
		...
		<td><?php echo date('d M, Y', strtotime($fetch['tgl_pajak'])) ?></td>
	</tr>
<?php
}
?>