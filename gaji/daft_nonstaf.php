...
?>
<section>
  <div class="activity-card" style="background-color:#F5F2EF;">
    <div class="col">
      <div class="row">
        <div class="col">
          <div class="form-inline d-flex">
            <h3>Daftar Data Karyawan Nonstaff</h3>
            <!-- <button class="btn btn-sm btn-warning ml-3 mb-2" data-toggle="modal" data-target="#myModal">tambah</button> -->
          </div>
        </div>
      </div><br>
      <form action="">
        <div class="row">
          <div class="col">
            <div class="form-inline d-flex">
              <a href="daftar.php" class="btn btn-sm btn-warning">Staff</a>
              <a href="daft_nonstaf.php" class="btn btn-sm btn-warning ml-2">Non staff</a>
            </div>
          </div>
          <div class=" form-inline d-flex">
            <div class="col">
              ...
              <select name="filter" id="filter" class="form-control form-control-sm" onchange="this.form.submit();">
                ...
                <?php
                $filt = mysqli_query($conn, "SELECT `cbg` FROM `daftar_pegawai`  WHERE `tipe` = 'nonstaff' GROUP BY `cbg` ORDER BY MIN(`cbg`) ASC") or die(mysqli_error($conn));
          ...
          <table class="table table-hover" id="respon">
            ...
            <tbody id="myTabl">
              ...
              if (isset($_POST['filter'])) {
                $tabel1 = mysqli_query($conn, "SELECT * FROM `daftar_pegawai` WHERE `cbg` = '$_POST[filter]' AND `tipe` = 'nonstaff' ORDER BY `nib` ASC, `ko_jbtn` DESC ") or die(mysqli_error($conn));
              }
              if (!isset($_POST['filter'])) {
                $tabel1 = mysqli_query($conn, "SELECT * FROM `daftar_pegawai` WHERE `tipe` = 'nonstaff' ORDER BY `nib` ASC") or die(mysqli_error($conn));
              }
              $no = 1;
              while ($lup = mysqli_fetch_array($tabel1)) {
                ...
                /// gaji pokok
                $gajii = mysqli_query($conn, "SELECT * FROM `gaji_gaji_golongan` WHERE `estimasi` = 'GAJI/' AND `tipe`= 'nonstaff'");
                $gaji_pk = mysqli_fetch_assoc($gajii);

                //kenaikan /thun
                $gaji = mysqli_query($conn, "SELECT * FROM `gaji_gaji_golongan` WHERE `estimasi` = 1 AND `tipe`= 'nonstaff'");
                $tm_g = mysqli_fetch_assoc($gaji);


                //tunjangan gaji
                $tunjg = mysqli_query($conn, "SELECT * FROM `gaji_gaji_golongan` WHERE `golongan` = '$kode' AND `tipe`= 'nonstaff'");
                $t_gaji = mysqli_fetch_assoc($tunjg);

                //get kebutuhan um
                //um interval
                $msakrj = $lup['ms_krj'];
                $r = mysqli_query($conn, "SELECT * FROM `gaji_tmbahum` WHERE `tipe`= 'nonstaff'");
                ...
                //um pokok
                $ums = mysqli_query($conn, "SELECT * FROM `gaji_um_golongan` WHERE `id` ='2' AND `tipe`= 'nonstaff'");
                $nilums = mysqli_fetch_assoc($ums); //tambahan um

                //tunjangan
                $tunjum = mysqli_query($conn, "SELECT * FROM `gaji_um_golongan` WHERE `golongan` ='$kode' AND `tipe`= 'nonstaff'");
              ...
              ?>
                <form action="index.php" method="POST">
                  ...
                  <td><?= $lup['nib'] ?></td>
                  <td>
                    <a href="edit_pegawai.php?id=<?= $lup['id']; ?>" style="font-weight:bolder; text-decoration:none; color:#4B4B4B;"><?= $lup['nama'] ?></a>
                  </td>
                  <td><?= $lup['tmpt_l'] . '. ' . tgl($lup['tgl_lahir']); ?></td>
                  <td><?= $umur->y; ?> thun</td>
                  ...
                  <td><?= $ket ?></td>
                  <td>
                    <a href="edit_pegawai.php?id=<?= $lup['id']; ?>" class="btn btn-sm aksi" style="background-color:#9DCBB8 ;"><span class="bx bxs-cog"></span></a>
                  </td>
                </form>
                </tr>
              <?php
