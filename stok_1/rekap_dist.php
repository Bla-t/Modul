...
<section class="recent">
  <div class="activity-card">
    ...
    <div class="form-inline mb-2">
      <h4 style="margin-top: 0.2rem;"> Laporan Distribusi</h4>
          ...
          <form method="POST" action="">
            ...
            <select class="form-control" name="tahun" id="" required>
              ...
              if (isset($_POST['tahun'])) {
                echo '<option value="' . date($_POST['tahun']) . '" selected>' . date($_POST['tahun']) . '</option>';
                for ($i = 2018; $i <= $a; $i++) {
    ...
    <!-- tampilan Default -->
    <?php
    $cex = mysqli_query($conn, "SELECT COUNT(id) AS 'jh' FROM `stok_isipakai` WHERE MONTH(tgl) = '$m'") or die(mysqli_error($conn));
    $iscex = mysqli_fetch_assoc($cex);
    if (!isset($_POST['lacak'])) {
    ?>
      <div class="row">
        <form method="GET" action="aksi/downcell30.php">
          ...
          <div class="form-inline mb-2">
            ...
            <button class="btn btn-success ml-1">
              <span class="ti-import">
                <input type="hidden" name="exportdst" value="<?= $milik; ?>">
              </span>
            </button>
          </div>
        </form>
    ...
    ?>
      ...
      <form method="GET" action="aksi/downcell30.php">
        ...
        <button class="btn btn-secondary ml-4" onclick="PrintDiv();" id="print"><span class="ti-printer"></span></button>
        <button class="btn btn-success ml-1" id="export"><span class="ti-import"><input type="hidden" name="exportdst" value="<?= $milik; ?>"></span></button>
      </form>
      </div>
      <div class="row">
        <p class="text-info"><?= $sts; ?></p>
        <div class="table table-responsive scroll" id="tabp">
          <table class=" table-hover">
            ...
            $n = 1;
            if ($level == "sudo") {
              $isi = mysqli_query($conn, " SELECT * FROM `$table` WHERE MONTH(tgl) = '$bul' AND YEAR(tgl) = '$thn' ORDER BY `tgl` ASC") or die(mysqli_error($conn));

              $TOT = mysqli_query($conn, "SELECT SUM(harga) AS 'tot' FROM `$table` WHERE MONTH(tgl) = '$bul' AND YEAR(tgl) = '$thn'");
            } else {
              $isi = mysqli_query($conn, " SELECT * FROM `$table` WHERE MONTH(tgl) = '$bul' AND YEAR(tgl) = '$thn' AND `milik`='$milik' ORDER BY `tgl` ASC") or die(mysqli_error($conn));

              $TOT = mysqli_query($conn, "SELECT SUM(harga) AS 'tot' FROM `$table` WHERE MONTH(tgl) = '$bul' AND YEAR(tgl) = '$thn' AND `milik` = '$milik'");
            }
            $has = mysqli_fetch_assoc($TOT);
            ?>
            <tbody id="table">
