    ...
    <div class="row">
      <form action="" method="POST">
        <div class="form-inline d-flex">
          ...
          <!--<input id="daerah" name="daerah" class="form-control form-control-sm ml-1" type="text" value="<?= $mil['daerah'] ?>">-->
          <!--<input type="hidden" name="id" value="<?= $mil['id'] ?>">&emsp;-->
        ...
        if (isset($_POST['ceks'])) {
          if ($_POST['sips'] == "dari") {
            ...
            mysqli_query($conn, "UPDATE `cek_dari` SET `kode`='$kode',`nama` = '$nama',`daerah`='$daerah',`stat` ='$status' WHERE `id`='$id'") or die(mysqli_error($conn));
            ...
            curl_close($curl);
                if ($result != "OK") { 
                    ...
                    echo "<script>window.location.href='tarifs.php?200';</script>";
                }

            exit;
          } elseif ($_POST['sips'] == "tujuan") {
            $id = $_POST['id'];
            $nama = $_POST['nama'];
            $nama_index = $_POST['nama_lama'];
            $kode = $_POST['kode'];
