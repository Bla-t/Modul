...
date_default_timezone_set('Asia/Jakarta');
	...
	//$dat= date('Y-m-d');
	$no=1;
	$query = mysqli_query($conn, "SELECT * FROM `dat_leasing` WHERE `Keterangan` NOT LIKE 'TERJUAL' AND `jenis_leasing`='ARMADA' ORDER BY `thn_pengada` ASC") or die(mysqli_error($conn));
